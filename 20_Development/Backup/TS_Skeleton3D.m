function A = TS_Skeleton3D(bw)
% 3D is TS_skeleton3D_v8, 2D is v7
% and see also TS_Skeleton3D_v6, it None Shaving data...
if ismatrix(bw)
    A = TS_Skeleton3D_v7(bw);
    return
end
...
% DistBW = imfilter(DistBW,ones(3,3,3));
% DistBW(~bw) = 0;
% ******* v8 Engine *********
...
for n = 1:Endn
    ...
    for k = 1:numel(newY)
        ...
        NowX = newX(k);
        NowZ = newZ(k);
        MaxDist = max(DistBW(NowY,NowX,NowZ),2);
