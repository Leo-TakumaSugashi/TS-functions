classdef Sugashi_Segment_v1p61
   ...
   properties
      ...
      SEGview_Type(1,:) char {mustBeMember(SEGview_Type,{'same','Type','Diameter','Length','class','AverageDiameter'})} = 'Type'  
      SEGview_SelectedLineWidth(1,1) double = 3;
...
%               MIPh = getappdata(fgh,'MIPhandles');
      ...
      %% Default set up
          ...
          function WindowResize(ParentH,event)
              ...
              FigPosi = [85        1126        1800         650];
                            
              NewPosi = ParentH.Position;
              if NewPosi(3) <FigPosi(3)
                  NewPosi(3) = FigPosi(3);
              end
              P1 = [1 1 720 NewPosi(4)];
              w = (NewPosi(3)-P1(3))/2;
              ...
              Aposi = [1, P1(4)-126.2, 716, 126.2];              
              Bposi = [1, P1(4)-126.2, w, 126.2];
              Cposi = [1, P1(4)-126.2, w, 126.2];
              H.ControllerA.Position = Aposi;
              H.ControllerB.Position = Bposi;
              H.ControllerC.Position = Cposi;
              
              H.ViewSEG_Pdata.Position = [1 1 716 P1(4)-126.2];
              H.Axes.Position = [1 1 w-2 P1(4)-2-Bposi(4)];
              H.View3DPanel.Position = [1 1 w-2 P1(4)-2-Cposi(4)];
              drawnow
              fprintf('Done\n')
          end
                  ...
                  case 'Select'                      
                      ...
                      if strcmpi(H.MenuH.Panel2_SegSelectionApply.Checked,'on')
                          ...
                          if znum > 1
                              ...
                              XLim = [min(xyz_2Dviewer(:,1))-AddX+.5  max(xyz_2Dviewer(:,1))+AddX+.5];
                              YLim = [min(xyz_2Dviewer(:,2))-AddY+.5  max(xyz_2Dviewer(:,2))+AddY+.5];
                              H.Axes.XLim = XLim;
                              H.Axes.YLim = YLim;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                              axis(H.Axes,'tight')
...
%                       NewSEG = obj.ReSegment(obj.Segment,'Separate',SelectedID,separate_index);
          ...
          function SetSliceView(~,~)
              ...
              H.SliderText(1).String = [...
                  num2str(NowSlice) '-' num2str(min(NowSlice+Num-1,znum)) '/' num2str(SliceNumel) '='...
                  num2str((NowSlice-1)*SliceReso(Dim),'%.0f') '-' num2str((min(NowSlice+Num-1,znum)-1)*SliceReso(Dim),'%.0f um') ];
...
%               PatchH(1).EdgeColor = 'b';
          ...
          function Check_SEGview_text(~,~)
              ...
              obj.ViewSupport.SEGview_textLim = 1000;
