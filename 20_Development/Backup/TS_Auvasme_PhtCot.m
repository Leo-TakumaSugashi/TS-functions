...
function output = TS_Auvasme_PhtCot(STR,Image,Reso)
...
try
 ...
 clear cbw skel3D
 TIME = tic;
 output = TS_AutoAnalysisDiam_PhtCot(RmfImage,skel_seg,NewReso,70);
...
[DiamImage,SNRImage,SImage] = TS_MeasDiam2DiamImage(skel_seg,output.Pointdata);
% R = TS_Diam2ReconstBW(DiamImage,mean(NewReso));

%%
NewDiamImage = DiamImage;
% NewDiamImage(SNRImage<2) = nan; %% This is For Sp5
% 10*log10(1.5849 ) == 2 [dB]
NewDiamImage(SImage<1.5849) = nan; %% This is For Sp8, PhotoCount
...
save NewSEGandDiamImage SEG_diam DiamImage SNRImage SImage NewDiamImage PDiamImage -v7.3

%% Reconstruct
disp('now Reconstruct....')
Dimvolume = false(size(NewDiamImage));
Ind = NewDiamImage>0;
Dim= NewDiamImage(Ind(:));
[y,x,z] = ind2sub(size(Ind),find(Ind(:)));
clear x y z
parfor i = 1:size(y,1)
    c = false(size(NewDiamImage));    
    c(y(i),x(i),z(i)) = true;
    r = round(double(Dim(i))/2);
    SE = strel('sphere',r);
    d = imdilate(c,SE);    
    Dimvolume = or(Dimvolume,d);
end

save Reconstruct Dimvolume NewDiamImage NewReso -v7.3
%% Finish and recording all time of Analysis
 AllTime = toc(AllTime);
