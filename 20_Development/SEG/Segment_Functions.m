...
%% Properties
    properties
        ...
        DistanceAsBranch(1,1) = 1; % Distance (Margin) in finding other branch.[um]
        LastDate = '2024/24th/Apr., by x-Leo'
        Version = '2.2.0' %% Stable version is 2.1.10, 
        UserData
    end
    methods
...
%                                 Pdata(n).NewXYZell(delete_index,:) = [];
            ...
            if ~isfield(Pdata,'DiameterByHand') || strcmp(OverWriteType,'f')
                for n = 1:length(Pdata)
                    Pdata(n).DiameterByHand = nan(size(Pdata(n).PointXYZ,1),1,'like',single(1));
                end
            end
            if ~isfield(Pdata,'DiameterByHandPosition') || strcmp(OverWriteType,'f')
                for n = 1:length(Pdata)
                    Pdata(n).DiameterByHandPosition = nan(size(Pdata(n).PointXYZ,1),5,'like',single(1));
                end
            end
            ...
            if ~isfield(Pdata,'NewXYZ') || strcmp(OverWriteType,'f')
                for n = 1:length(Pdata)
                    Pdata(n).NewXYZ= nan(size(Pdata(n).PointXYZ,1),5,'like',single(1));
                end
            elseif size(Pdata(1).NewXYZ,2) ~=5
                for n = 1:length(Pdata)
                    Pdata(n).NewXYZ= nan(size(Pdata(n).PointXYZ,1),5,'like',single(1));
                end
                % % if after resampling....
            ...
            if ~isfield(Pdata,'NewXYZrot') || strcmp(OverWriteType,'f')
                for n = 1:length(Pdata)
                    Pdata(n).NewXYZrot= nan(size(Pdata(n).PointXYZ,1),5,'like',single(1));
                end
            elseif size(Pdata(1).NewXYZrot,2) ~=5
                for n = 1:length(Pdata)
                    Pdata(n).NewXYZrot= nan(size(Pdata(n).PointXYZ,1),5,'like',single(1));
                end
                % % if after resampling....
            ...
            if ~isfield(Pdata,'NewXYZnor') || strcmp(OverWriteType,'f')
                for n = 1:length(Pdata)
                    Pdata(n).NewXYZnor= nan(size(Pdata(n).PointXYZ,1),5,'like',single(1));
                end
            elseif size(Pdata(1).NewXYZnor,2) ~=5
                for n = 1:length(Pdata)
                    Pdata(n).NewXYZnor= nan(size(Pdata(n).PointXYZ,1),5,'like',single(1));
                end
                % % if after resampling....
            ...
            if ~isfield(SEG,'BranchPointTable')
                SEG.BranchPointTable = obj.MakeBranchTable(SEG);
            elseif and(strcmp(OverWriteType,'f'),isfield(SEG,'BranchPointTable'))
                SEG.BranchPointTable = obj.MakeBranchTable(SEG);
            end
            if ~isfield(SEG,'BPmatrix')
                SEG.BPmatrix = obj.Modify_BranchPointMatrix(SEG);
                disp('   BPmatrix will be remove....')
                disp('   ==== New Matrix is BranchPointTable!! =====')
            end
            ...
            for nf = 1:length(PFields)
                ...
                if and(and(or(isnumeric(Xn),islogical(Xn)),size(Xn,1)>1),...
                    ...
                    X(Delete_Ind,:) = [];
                        ...
                        warning(err.message)
                        fprintf(['Error Fields Name in Pointdata is "' PFields{nf} '"'])
                        if contains(PFields{nf} , 'GenerationsNum')
                            fprintf('  +==>  Field will be empty cell.\n')
                            X = [];
                        else
                            keyboard
                        end
...
%             NewPdata.ID       = max(max(abs(cat(1,NewPdata.ID))),OldMaxID)+1
            ...
            NewPdata.MEMO     = [NewPdata.MEMO, '[Paired:' num2str(ID,'%.0f ') ,']'];
        end
        function BranchTable = MakeBranchTable(obj,SEG)
            Reso = SEG.ResolutionXYZ;
            D = obj.DistanceAsBranch;
            catID = cat(1,SEG.Pointdata.ID);
            EnableID = sum(catID>0);
            Pdata = SEG.Pointdata(catID>0);
            xyz = zeros(EnableID*2,3);
            IDs = xyz(:,1);
            for n = 1:length(Pdata)
                p = Pdata(n).PointXYZ([1 end],:);
                xyz((n-1)*2+1:n*2,:) = p(1:2,:);
                IDs((n-1)*2+1:n*2,1) = Pdata(n).ID;
                Pdata(n).ID_numels = ...
                    repmat(Pdata(n).ID,[size(Pdata(n).PointXYZ,1),1]);
            end
            IDc = num2cell(uint32(IDs));
            TF = 1;
            t = uint32(1); % taget number
            % % % % 1st. unique Branch Point
            fprintf('  >>1st. Find Unique BP...')
            while TF
                txyz = xyz(t,:);
                oxyz = xyz(t+1:end,:);
                len = obj.GetEachLength(txyz,oxyz,Reso);
                Ind = find(len<=D);
                if ~isempty(Ind)
                    Ind = Ind + feval(class(Ind),t);
                    xyz(t,:) = nanmean(xyz([t;Ind],:),1);
                    IDc{t} = IDs(uint32([t;Ind]))';
                    xyz(Ind,:) = [];
                    IDs(Ind) = [];
                end
                t = t + uint32(1);
            ...
            IDc(length(IDs)+1:end) = [];
            % % % % 2nd. Add Middle Position Segment ID
            AllID = cat(1,Pdata.ID_numels);
            XYZ = cat(1,Pdata.PointXYZ);
            ParentID = cell(size(IDc));
            fprintf('Done. \n  >>2nd. Checking Middle Position... \n')
            TS_WaiteProgress(0)
            PW = [0 100];
            for n = 1:size(xyz,1)
                txyz = xyz(n,:);
                len = obj.GetEachLength(txyz,XYZ,Reso);
                checkIDs = AllID(len<=D);
                ParentID{n} = unique(checkIDs');
                if PW(1) == PW(2)
                    PW(1) = 0;
                    TS_WaiteProgress(n/size(xyz,1))
                else
                    PW(1) = PW(1) + 1;
                end
            end
            ...
            y = xyz(:,2);
            z = xyz(:,3);
            BranchTable = table(x,y,z,ParentID);
...
%                             peace_xyz,Reso,Step,xy_range,z_range);
                    ...
                    [NewXYZ,ERR] = resampling(obj,xyz,Reso,Step,xy_range,z_range);
                    if ERR 
                        disp([ 'check ID : ' num2str(Pdata(n).ID)])
                    end
                end
            ...
            function [Newxyz,ERR] = resampling(obj,xyz,Reso,Step,xyR,zR)
                ...
                NaNind = find(max(isnan(xyz),[],2));
                
                if ~isempty(NaNind)
                    disp('Find NaN in input xyz...')
                    xyz(NaNind,:) = [];
                    ERR = true;
                end
...
%                 fprintf(['      IDs : ' num2str(reshape(catID(chase_index),1,[])) '\n'])
        ...
        function [IDs,Dist,SeparateXYZ] = Find_NearSegment_xyz(obj,xyz,Minimum)
            ...
            if and(size(xyz,1)~=1,~isvector(xyz))
                error('Input is not correctfsd')
            end
            SEG = obj.Segment;
%             selfID = obj.FindID_xyz(xyz);
%             selfIndex = obj.ID2Index(selfID);
%             try
%                 for abs_n = 1:length(selfIndex)
%                     SEG.Pointdata(selfIndex(abs_n)).ID = abs(SEG.Pointdata(selfIndex(abs_n)).ID)*(-1);
%                 end
%             catch eee
%                 keyboard
%             end
            
            deleteID = cat(1,SEG.Pointdata.ID) < 0;
            SEG.Pointdata(deleteID) = [];
            XYZ = cat(1,SEG.Pointdata.PointXYZ);
            
            catID = zeros(size(XYZ,1),1);
            s = 1;
            for n = 1:length(SEG.Pointdata)
                e = size(SEG.Pointdata(n).PointXYZ,1);
                catID(s:s-1+e) = repmat(SEG.Pointdata(n).ID,[e,1]);
                s = s+e;
            end
...
%             keyboard
            ...
            if sum(ind) == 0
                selfID = obj.FindID_xyz(xyz);
                msg = {['NOT found segment in ' num2str(Minimum) ' µm.' ],...
                    ['pls try check ID(s) :' num2str(selfID)]};
                warndlg(msg,'WARNING:Branch Editor','modal')
                IDs = [];
                Dist = [];
                SeparateXYZ = [];
                return
            ...
            IDs = unique(catID(ind));
            Dist = zeros(size(IDs));
            SeparateXYZ = zeros(length(IDs),3);
            for n = 1:length(IDs)
                check_ind = catID ==IDs(n);
                check_xyz = XYZ(check_ind,:);
                check_len = len(check_ind);
                [v,xyzind] = min(check_len);
                Dist(n) = v;
                SeparateXYZ(n,:) = check_xyz(xyzind,:);
            end
        end
