function [output,ppdata] = TS_AutoAnalysisDiam_SEG_v2024Alpha(...
...
for n = 1:length(SEG.Pointdata)
    ...
    pxyz2 =  nan(len,5,siz(4),'like',single(1));
    ...
    SEG.Pointdata(n).NewXYZrot = pxyz2;
    SEG.Pointdata(n).NewXYZnor = pxyz2;
...
%% Analysis Noise and Measure Diameter
    ...
    for k = 1:length(NewStep)
        ...
        if max(strcmp(Option.MeasureType,{'LineRot','Hybrid','All'}))
            ...
            LineRotNewXYZ = nan(length(LineRotInd),5);
            ...
            parfor n = 1:length(LineRotInd)
                ...
                if TF
                    ...
                    LineRotNewXYZ(n,:) = [newx(1) newy(1) newx(2) newy(2) newz];
                end
            end
            %% output
...
%                     vpY = Yq(:,n) + indY(NormInd(n));
                    ...
                    parpdata(n).NewXYZnor = [newx(1) newy(1) newx(2) newy(2) newz]; 
                else
                    %% output for parpdata
                    parpdata(NormInd(n)).NormDiameter = nan;
                    parpdata(n).NewXYZnor = [indX(n), indY(n), indX(n), indY(n), indZ(n)];        
                end
            end       
        end
    ...
    for Nind = 1:length(Number)
        ...
        Theta = Signal;
        NewXYZrot = zeros(length(Signal),5);    
        NewXYZnor = zeros(length(Signal),5);    
        NewXYZell = zeros(length(Signal),3);    
        for k = 1:size(xyz,1)
            ...
            NewXYZrot(k,:) = EmptyCheck(ppdata(c).NewXYZrot,5);
            NewXYZnor(k,:) = EmptyCheck(ppdata(c).NewXYZnor,5);
        ...
        NewXYZrot = (NewXYZrot -1 ) .* Reso([1 2 1 2 3]);
        NewXYZrot = (NewXYZrot ./ SEGRESO([1 2 1 2 3]) ) + 1;
        NewXYZnor = (NewXYZnor -1 ) .* Reso([1 2 1 2 3]);
        NewXYZnor = (NewXYZnor ./ SEGRESO([1 2 1 2 3]) ) + 1;
...
elseif siz(4)>1
    ...
    n = 4654;
    [Xq,Yq,~] = GetSignalMesh(indX(n),indY(n),indZ(n),siz);
    keyboard
...
%                     vpY = Yq(:,n) + indY(NormInd(n));
       ...
       for Nind = 1:length(Number)
           ...
           for kt = 1:size(xyz,1)
               ...
               NewXYZrot(kt,:) = EmptyCheck(parpdata(c).NewXYZrot,5);
               NewXYZnor(kt,:) = EmptyCheck(parpdata(c).NewXYZnor,5);
...
for n = 1:length(Number)
    output.Pointdata(Number(n)) = SEG.Pointdata(Number(n));
end
% % % % % % % % % % % % % % % % % % % % 
