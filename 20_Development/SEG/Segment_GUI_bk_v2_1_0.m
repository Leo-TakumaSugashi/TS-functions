classdef Segment_GUI_bk_v2_1_0
   ...
   properties
      ...
      Tag
      Version = '2.1.0' % backup is 2024/04/24
      UserData
   end
   methods
       %% Input check
...
%           H.figure.Name = ['Segment Editor (ver. '  num2str(VersionNum,'%.1f')  ')'];
          ...
          set(H.figure,'WindowButtonMotionFcn',@WindowButtonFcn)
          set(H.figure,'WindowButtonUpFcn',@WindowButtonFcn)
          set(H.figure,'WindowScrollWheelFcn',@WindowButtonFcn)
          ...
          function WindowButtonFcn(fgh,event) 
              % % % ADD New Point %%%%
              ...
              P2 = get(H.Panel(2),'Position');
              P2x = [P2(1),P2(1)+P2(3)];
              P2y = [P2(2),P2(2)+P2(4)];
              P3 = get(H.Panel(3),'Position');
              P3x = [P3(1),P3(1)+P3(3)];
              P3y = [P3(2),P3(2)+P3(4)];
              P0 = get(H.figure,'CurrentPoint');
              P0 = P0(1,[1 2]);
              TF2Dx = and(P0(1)>P2x(1),P0(1)<P2x(2));
              TF2Dy = and(P0(2)>P2y(1),P0(2)<P2y(2));
              TF2D = and(TF2Dx,TF2Dy);
              TF3Dx = and(P0(1)>P3x(1),P0(1)<P3x(2));
              TF3Dy = and(P0(2)>P3y(1),P0(2)<P3y(2));
              TF3D = and(TF3Dx,TF3Dy);
              
              try
                  gco_type = get(gco,'Type');
              catch 
                  gco_type = [];
              end
              if and(gca == H.Axes,TF2D)
                  if isempty(gco) % images.roi.(class)
                      return
                  elseif ~strcmpi(gco_type,'image')
                      return
                  end
                  switch event.EventName
                      case 'WindowMouseMotion'
                          if MD.Press
                              ...
                              xlim(gca,MD.PreXLim+DIFF(1))
                              ylim(gca,MD.PreYLim+DIFF(2))
                          end
              ...
              elseif and(gca == H.View3DAxes,TF3D)
                  switch event.EventName
                      case 'WindowMouseMotion'
                          if MD.Press
          ...
          function WindowKeyFcn(fgh,event)
...
%               fprintf([event.EventName '.   Please Wait...'])
              ...
              %%%%%%%%%%%%%%%%%%%%%% CHECK POINT ##############
              if (P1(4)-2-Bposi(4)) > 580
                H.Axes.Position(2) = -30;
              end
              ...
              if strcmpi('with Volume',obj.ViewerType)
                  ...
                  if ~strcmpi('normalized',H.View3_BasePanel.Units)
                      H.View3_BasePanel.Units = 'normalized';
                      H.Rendering.Units = 'normalized';
                      H.BranchEditPanel.Units = 'normalized';

                      H.View3_BasePanel.Position = [0.00,  0.00,  0.40,    0.99];
                      H.Rendering.Position = [0.41,  0.0,  0.58,   0.99];
                      H.BranchEditPanel.Position = [0.41   0.0   0.58    0.99];
                  end
              end              
          end
...
%               oh.Enable = 'off';
              ...
              [StepSize,LimNum,LimLen,PredictionTF,A,Opt,CenterOpt] = Read_HandMeasureOPT;
...
%                       Posi(newindex,:) = output.Pointdata.NewXYZnor;
              ...
              Index(Ind) = true;
              Data = set_Table_MeasurePoint(Index,Centers,theta,true,false,HandDiam);
              ...
              MeasureData = settingup_userdata(P,Centers,...
                  true(size(P,1),1),false(size(P,1),1),theta);
          ...
          function [StepSize,LimNum,LimLen,PredictionTF,A,Opt,CenterOpt]...
                  = Read_HandMeasureOPT
          ...
          function CallBack_HandMeasureCellSelection(a,b)
              fprintf(['   Don', '''' ,'t touch me!!\n'])
          end
...
%                       EnableInd = or(EnableInd,ADD_EnableInd);
                  ...
                  if ~Smooth_skipTF
                      ...
                      SEG = obj.SEGFcn.set_Segment(SEG,'f');
