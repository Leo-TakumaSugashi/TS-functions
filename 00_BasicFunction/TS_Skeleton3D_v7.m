function A = TS_Skeleton3D_v7(bw)
% This Func is using by Each slice (bwdist)
...
DistBW = zeros(size(bw),'like',single(1));
for n = 1:size(bw,3)
    DistBW(:,:,n) =  bwdist(~bw(:,:,n));
end
...
% close(waitbarh)
TIMEout = toc(timeval);
disp(['Elapsed time ' num2str(floor(TIMEout/60)) ' min. ' num2str(TIMEout-60*floor(TIMEout/60)) ' sec.'])
