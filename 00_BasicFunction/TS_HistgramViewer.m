function TS_HistgramViewer(data)
...
for n = 1:CH
    ...
    bh(n) = patch('Parent',axh2,'Xdata',x(:),'Ydata',max(y(:),1),...
...
set(axh2,'Xlim',[-diff(X(1:2))+X(1) X(end)+diff(X(1:2))],'Ylim',[1 MaximumFreq],...
...
set(Climh,'Visible','off','Userdata',n) %% Userdata is channels number
clear Climh ch a 
end
...
%% Axis T
    slh(2) = uicontrol('Style','slider',...
        'Unit','Normalized',...
        'BackgroundColor',[0.5 0.5 0.5],...
        'Position',[.32 0 .25 0.04],...
        'Value',0,...
        'Callback',@Callback_slider,...
        'Userdata',T-1);
    txh(2) = uicontrol('Style','text',...
        'unit','Normalized',...
        'Position',[0.32 0.04 0.1 0.03],...
        'String','T: 1');
%% Axis CH
    slh(3) = uicontrol(...
        'Unit','Normalized',...
...
%% Now Edit Ch
    slh(4) = uicontrol(...
        'Unit','Normalized',...
        'BackgroundColor',[0.8 0.8 0.8],...
        'Position',[.82 0 .18 0.05],...
        'Callback',@Callback_NowEditChannels,...
        'Userdata',1,...
        'String','Edit Channels: 1');
    guidata(slh(4),CH)
...
if CH>1, set(slh(3:4),'Visible','on'),end
...
Callback_slider(oh)
 ...
 %% Callback NowEditChannels
 function Callback_NowEditChannels(oh,~)
 maxCH = guidata(oh);
 fgh = oh.Parent;
 Climdata = getappdata(fgh,'Climh');
 DataName = cell(maxCH,1);
for n=1:maxCH,DataName{n} = num2str(n);   end
[CH,ok] = listdlg('PromptString','Select Channel (single)',...
    'SelectionMode','single','ListString',DataName);
if ok
    oh.String = ['Edit Channel :' num2str(CH)];
    axh = get(Climdata(CH).Handle,'Parent');
    ch = get(axh,'children');
%      disp(ch)
    ClimHandles = ch(1:maxCH);
    HistgramH = ch(maxCH+1:end);
     Number = cell2mat(get(ClimHandles,'Userdata'));
     Ind = Number == CH;     
     set(axh,'Children',[ClimHandles(Ind); ClimHandles(~Ind); HistgramH])
end
...
for n = 1:CH
    Index = sub2ind([1 1 Z T CH],1,1,NowSlice,NowTime,n);
    [x,y] = Bar2patchdata(HistMap(Index).HistCenter,HistMap(Index).Hist);
%     set(bh(n),'Ydata',y(:),'Xdata',x(:)) %% Type Normal
    set(bh(n),'Ydata',max(y(:),1),'Xdata',x(:)) %% Type log
    %% imrect chenge
    H = Climdata(n).Handle;
    Posi = getPosition(H);    
    NowClim = HistMap(Index).Clim;
    Posi(1) = NowClim(1);
...
if length(NowChannels) == 1
    ...
    Ind = false(CH,1);Ind(NowChannels) = true;
    torgb(:,:,~Ind) = 0; 
    Image = squeeze(torgb);
%     Ind = Ind(NowChannels);
    Min = [];Max = [];
    for n = 1:length(NowChannels)
        n_ch = sub2ind([1 1 Z T CH],1,1,NowSlice,NowTime,NowChannels(n));
        X = HistMap(n_ch).HistCenter;
...
axh = getappdata(fgh,'Axes');
% set(axh(2),'Ylim',[0 Ylim]) %% Type normal
set(axh(2),'Ylim',[1 Ylim]) %% type Log
% Clim Ylimit
...
if isempty(Children)
    ...
    Children = figure('Posi',Pposi,...
        ...
        'Menubar','None',...
        'NumberTitle','off',...
        'Name',['Parent:' num2str(fgh.Number) ' - Clim Editer'],...
    ...
    function Callback_ClimEdit(oh,~)
        ...
        for n_ch = 1:CH        
            ...
            Xdata = double(get(plh_min(n_ch),'Xdata'));
            xslice = 1:Z;
            %% Clim Interplotion
