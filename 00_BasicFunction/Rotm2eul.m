function eul = Rotm2eul(rotm, sequence)
    ...
    switch sequence
        case 'ZYX'
            ...
            if (rotm(3,1) < 1)
                if (rotm(3,1) > -1) % case 1: if r31 ~= ±1
                    ...
                    eul(1) = atan2(rotm(2,1), rotm(1,1)); % theta_z
                    eul(2) = asin(-rotm(3,1));            % theta_y
                    eul(3) = atan2(rotm(3,2), rotm(3,3)); % theta_x
        ...
        case 'ZYZ'
            ...
            if (rotm(3,3) < 1)
                if (rotm(3,3) > -1)
                    ...
                    eul(1) = atan2(rotm(2,3),  rotm(1,3)); % theta_z1
                    eul(2) = acos(rotm(3,3));              % theta_y (is equivalent to atan2(sqrt(r13^2 + r23^2), r33) )
                    eul(3) = atan2(rotm(3,2), -rotm(3,1)); % theta_z2
                else % if r33 = -1:
                    % Gimbal lock: infinity number of solutions for
                    %   theta_z2 - theta_z1 = atan2(r21, r22), --> set theta_z2 = 0.
                    eul(1) = -atan2(rotm(2,1), rotm(2,2)); % theta_z1
                    eul(2) = pi;                           % theta_y
                    eul(3) = 0;                            % theta_z2
                end
            else % if r33 = 1:
        ...
        %     end
        case 'XYZ'
            eul(1) = atan2(-rotm(2,3),rotm(3,3));
            eul(2) = asin(rotm(1,3));
            eul(3) = atan2(-rotm(1,2),rotm(1,1));

        case 'XZY'
            eul(1) = atan2(rotm(3,2),rotm(2,2));
            eul(2) = -asin(rotm(1,2));
            eul(3) = atan2(rotm(1,3),rotm(1,1));

        case 'YXZ'
            eul(1) = atan2(rotm(1,3),rotm(3,3));
            eul(2) = -asin(rotm(2,3));
            eul(3) = atan2(rotm(2,1),rotm(2,2));

        case 'YZX'
            eul(1) = atan2(-rotm(3,1),rotm(1,1));
            eul(2) = asin(rotm(2,1));
            eul(3) = atan2(-rotm(2,3),rotm(2,2));

        case 'ZXY'
            eul(1) = atan2(-rotm(1,2),rotm(2,2));
            eul(2) = asin(rotm(3,2));
            eul(3) = atan2(-rotm(3,1),rotm(3,3));
