function varargout = TS_3dmipviewer_old2016(data,varargin)
...
siz = size(data);
if max(siz(1:2))>MaximumSiz
    data = imresize(data,MaximumSiz/max(siz(1:2)));
end
if siz(3) > MaximumSiz
    data = TS_imresize3d(data,[size(data,1) size(data,2) MaximumSiz]);
end
siz = size(data)-1;
...
if islogical(im64)
    A = rgbproj(squeeze( TS_3DMIP_view3_bw(im64,Reso64,Theta,Fai)));
else
...
if islogical(data)
    A = rgbproj(squeeze( TS_3DMIP_view3_bw(data,Reso,Theta,Fai)));
else
    A = rgbproj(squeeze( TS_3DMIP_view3(data,Reso,Theta,Fai)));
end
[A,NewSiz] = TS_padding0(MaxSiz(1:2),A);
imh.CData = A;
