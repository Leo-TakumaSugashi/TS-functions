function handles = TS_SEGview(SEG,varargin)
% handles = TS_SEGview(SEG,{Vieweing Type})
% edit by sugashi , input viewing type 
% 
% see also , TS_AutoSegment_loop, TS_SEGandDiam, DiamImage(dim) = TS_MeasDiam2DiamImage
...
for n = 1:length(SEG.Pointdata)
   ...
   mapind = max(round(Len(n)),1);
   mapind = min(mapind,size(map,1));
