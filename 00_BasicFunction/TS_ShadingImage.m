function [New_output,Divisor,output] = TS_ShadingImage(Image,Reso)
...
%   fgh = figure('Color','w','Name','Now, Shading Processing of Input Image');centerfig(fgh)
...
%% making Divisor
FiltImage = zeros(size(Image),'like',single(1));
parfor n = 1:size(Image,3)
FiltImage(:,:,n) = imfilter(single(Image(:,:,n)),fs_gauss,Fil_term);
end
Divisor = FiltImage;
...
parfor n = 1:size(Image,3)
    ...
    output_im = output(:,:,n) - FiltImage(:,:,n); %% Like TopHat filter...
