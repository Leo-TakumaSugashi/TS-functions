function Output = TS_AtSEG1(Output,bwthindata,BPcentroid,BPgroup,L_BPgroup,EndP,BPmatrix)
...
for n = 1:length(ey)
    ...
    Z = ez(n);
    BranchPoint = nan(1,3);
    % % End point check(End-End‚Í“ñ”Ô–Ú‚ÌEndPoint‚ðÁ‚µ‚Ä‚µ‚Ü‚¤‚½‚ßB)
    if not(bwthindata(Y,X,Z))
        continue
    end
    c = uint32(1);
    Segment = struct('point',[]);
    Segment(c).point = double([X Y Z]-1); %% padarray‚µ‚Ä‚¢‚é‚½‚ß-1
