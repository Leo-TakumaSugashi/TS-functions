function SEG = TS_AutoSegment_v2021b(skel,NewReso,AddBP,cutlen)
...
BPgroup = bwmorph3(bwthindata,'branchpoint');
BPcentroid = BPGroup2BPCentroid(BPgroup);
EndP = bwmorph3(bwthindata,'endpoint');
%%%%% 2021b modified%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%<-
...
for n = 1:length(ey)
    ...
    X = ex(n);
    Z = ez(n);
%     hold(axh,'on'),plot3(axh,X,Y,Z,'o'),keyboard%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    BranchPoint = nan(1,3);
...
NewEndP = bwmorph3(bwthindata,'endpoint');
%%%%% 2021b modified%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%<-
...
function outputBP = BPGroup2BPCentroid(BPG)
LBP = bwlabeln(BPG,26);
s = regionprops(LBP,'Centroid');
outputBP = false(size(BPG));
% % Nearest to Centroid Point
for n = 1:length(s)
    [y,x,z] = ind2sub(size(BPG),find(LBP(:)==n));    
    if length(y)==1
        outputBP(y,x,z) = true;
        continue
    else
