function [angle,utheta,uvar] = hybridvel(inputimg,showimg,saveimg,delx,delt,hi,lineskip,xrange,ID)
...
if showimg
    imgtitle = {inputname(1);['HybridVel-ID ',num2str(ID)]};
end
%% Process the image with different filters
curimtitle = {'Demeaned image','Vert. demean','Sobel filtering'}; curcolor = {'b','g','k','r','c','m'};
...
figure,subplot(1,3,1),imagesc(imgseg(:,:,1)),subplot(1,3,2),imagesc(imgseg(:,:,2)),...
    subplot(1,3,3),imagesc(imgseg(:,:,3))
...
for ii = 1:imgsegsz(3) % eachimg(1: demean 2: Vertical demean 3:sobel)
    for jj = 1:segn
    ...
    if showimg
        ...
        imagesc(imgseg(:,:,ii)); axis image; title(curimtitle{ii}); colormap(gray)
        ...
        if ii==1
            ylabel(['\Deltat = ' num2str(round(delt,1)) ...
                ' ms/frame, h = ' num2str(imgsegsz(1)) ' pixels']);
            
        end
        if ii == 2
            xlabel(['w = ' num2str(wi),...
                ' pixels; \Deltax = ' num2str(delx) ' \mum/pixel']);
        end
