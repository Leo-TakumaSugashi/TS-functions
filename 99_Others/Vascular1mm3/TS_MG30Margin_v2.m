function output = TS_MG30Margin_v2(im1,im2,im3,im4)
...
im2 = padarray(im2,[500-5     12],'post');
im2 = padarray(im2,[5     500-12],'pre');
ImageAll_12 = max(cat(4,im1,im2),[],4);
clear im1 im2

%% loc 4 and 3
im3 = padarray(im3,[500 500],'pre');
im4 = padarray(im4,[500 24],'pre');
im4 = padarray(im4,[0 500-24],'post');
ImageAll_43 = max(cat(4,im4,im3),[],4);
clear m3 m4 im3 im4
%% margin 12 and 43
  %% XY [25 -11]
ImageAll_12 = padarray(ImageAll_12,[0 3],'pre');
ImageAll_43 = padarray(ImageAll_43,[0 3],'post');
ImageAll_43(1:25,:,:) = [];
ImageAll_43 = padarray(ImageAll_43,[25 0],'post');
output = max(cat(4,ImageAll_12,ImageAll_43),[],4);
clear ImageAll_12 ImageAll_43
load('NoneDataArea.mat','x_v2','y_v2')
output(~y_v2,:,:) = [];
output(:,~x_v2,:) = [];
