function Output = TS_AutoSegment1st_proto(bwthindata,varargin)
...
[~,~,BPgroup,EndP] = TS_bwmorph3d(bwthindata,'branchpoint','none');

%% Add Shaving minimum pixels length(nearest of 26)
DeletePoint = or(and(imdilate(EndP,ones(3,3,3)),EndP),...
    and(imdilate(BPgroup,ones(3,3,3)),EndP));
DeletePoint = or(DeletePoint,and(imfilter(uint8(EndP),ones(3,3,3))==1,EndP));
    
    disp(['Shaving of  Minimum length ..'  num2str(sum(DeletePoint(:))) ...
        ' / ' num2str(sum(bwthindata(:))) ' = ' ...
        num2str(sum(DeletePoint(:))/sum(bwthindata(:)))])
%     ph = pointview(bwthindata,Reso,'figure');
%     hold on
%     ph(2) = pointview(DeletePoint,Reso);
%     set(ph(2),'Marker','o','Color','r')
...
for n = 1:length(NewEy)
    ...
    %% Find Nearest BPgroup ********* step 2(branch-branch).1.(branch??<--Point-->2.2.)
    % 3x3x3 crop
    BPcentroid_ROI = BPcentroid(Y-1:Y+1,X-1:X+1,Z-1:Z+1);
    BPgroup_ROI = BPgroup(Y-1:Y+1,X-1:X+1,Z-1:Z+1);
    bwthindata(Y,X,Z) = false;
...
%             p = repmat([Y1 X1 Z1],[size(Fyxz,1) 1]);
    ...
    %% Next point ****************** step 2(branch-branch).2.(branch-Point-->?) 
    ...
    if max(BPgroup_ROI(:))
        ...
        if BPcentroid(Y1,X1,Z1)
            ...
            % Find Nearest BPpoint(centorid)        
            FindNearestBP = and(BPcentroid,L_BPgroup==Lnum);
            if max(FindNearestBP(:))            
            [Fy,Fx,Fz] = ind2sub(size(FindNearestBP),find(FindNearestBP(:)));
            Fyxz = cat(2,Fy,Fx,Fz);
            else
            disp('May be , Bag......')
            disp(['*****Current Pointdata Number is ' num2str(Pdata_count) '*****'])
            clearBPcent = BPcentroid;
            bpxyz= BranchPoint(1,:) +1;
             clearBPcent(bpxyz(2),bpxyz(1),bpxyz(3)) = false;
             [Fy,Fx,Fz] = ind2sub(size(clearBPcent),find(clearBPcent(:)));
             clear clearBPcent
             Fyxz = cat(2,Fy,Fx,Fz);            
            end           
            p = repmat([Y1 X1 Z1],[size(Fyxz,1) 1]);
...
%                         p = repmat([Y1 X1 Z1],[size(Fyxz,1) 1]);
                ...
                if max(ROI(:))
                    ...
                    clear clearBPcent
                        ...
                        Segment(c).point = double([X1 Y1 Z1]-1);
                        BranchPoint(2,:) = double([X1 Y1 Z1]-1);
                        clear lenp LEN Ind X1 Y1 Z1 Fyxz p   
                end
            end
            clear Fyxz Ind LEN p Fy Fx Fz FindNearestBP X1 X2 Y1 Y2 Z1 Z2 
            clear BPGy BPGx BPGz BPG lenp
