function [output,ppdata] = TS_AutoAnalysisDiam_SEG_v2019delta(fImage,inReso,ThresholdType,SEG,varargin)
...
if and( size(fImage,3) == 1, size(fImage,4) > 1)
    for k = 1:size(fImage,4)
        outputSlice = TS_AutoAnalysisDiam_SEG_v2019delta(...
...
SingleByte = 4;
MaximumUsableMemorySize = 15 * 2^30; % Giga Bytes
EachStep = floor(MaximumUsableMemorySize / ( prod(siz(1:2)) * SingleByte * NumWorker^2 ));
...
%         if n == 30
%             fgh = figure('Posi',[30 30 900 500]);
%             axes('Posi',[0 0 .5 1])
%             imagesc(im.*double(rArea)),axis image off,
%             colormap(kjet)
%             axes('Posi',[0.55 0.1 .35 .8])
%             histogram(gca,im(rArea))
%             keyboard
%         end
    ...
    if siz(3)>1
        ...
        if siz(3)>1
            ...
            parZImage = interp2(fImage,Xq,Yq,'bilinear');
        end
        parfor n = 1:length(indZ)
            LineZ = parZImage(:,n);
            LineZ(isnan(LineZ)) = 0;
