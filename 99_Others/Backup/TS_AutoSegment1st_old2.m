function Output = TS_AutoSegment1st_old2(bwthindata,varargin)
...
[BPcentroid,~,BPgroup] = TS_bwmorph3d(bwthindata,'branchpoint','none');
...
for n = 1:length(ey)
    ...
    while Go2Next
        ...
        elseif max(BPcentroid_ROI(:))
            ...
            lenp = padarray(lenp,[size(BPC,1)-1 0],'symmetric','pre');
           ...
           if max(ROI(:))==0
            ...
            lenp = padarray(lenp,[size(C,1)-1 0],'symmetric','pre');
            LEN = sum((C - lenp).^2,2);
            [~,Ind] = min(LEN);
            Ind = Ind(1);
            X = C(Ind,2);
...
for n = 1:length(NewEy)
    ...
    if max(BPgroup_ROI(:))
        [BPGy,BPGx,BPGz] = ind2sub(size(BPgroup_ROI),find(BPgroup_ROI(:)));
        BPG = cat(2,BPGy+Y-2,BPGx+X-2,BPGz+Z-2);
        lenp = padarray([Y X Z],[size(BPG,1)-1 0],'symmetric','pre');
        LEN = sum((BPG - lenp).^2,2);
        [~,Ind] = min(LEN);
        Ind = Ind(1);
        X1 = BPG(Ind,2);
    ...
    elseif max(ROI(:))
        ...
        while Go2Next
            ...
            clear Ny Nx Nz
            lenp = padarray([Y X Z],[size(NextPoint,1)-1 0],'symmetric','pre');
