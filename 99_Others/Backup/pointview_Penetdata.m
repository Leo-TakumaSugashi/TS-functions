function varargout  = pointview_Penetdata(Penetdata,varargin)
% varargout  = pointview(thindata,Reso,{Maximum},{'figure'})
% BIN = 32;
% [h,x] = hist(Diam_skel(thindata),BIN);
narginchk(1,2)
siz = Penetdata.Size;
skelind = Penetdata.skel_ind;
dimind = Penetdata.Diameter;
Reso = Penetdata.Resolution;

thindata = false(siz);
thindata(skelind) = true;

Diam_skel = zeros(siz,'single');
Diam_skel(skelind) = dimind;
