classdef Sugashi_Segment_v1p1
   properties
      Name
      Value
      Image %% cell for tracking ()
      Resolution(1,3)
      MaxChannels = 12; %% will be for tracking
      SEGdata %% cell for tracking ()
      Segment
      UserData
   end
   methods
      ...
      function obj = set.Image(obj,VC)
          % *Image data has to be cell class
          if ~iscell(VC)
             error('Input Image data must be a (or 1xn size) cell.')
          end
          if ~isvector(VC)
              error('Input Image data Size must be "1 x n".')
          end
          fprintf('#### Setting up Image file. ################\n')
          for n = 1:length(VC)
              v = VC{n};
              disp(['    Channels: ' num2str(n)])
              disp(['    class   : ' class(v)])
